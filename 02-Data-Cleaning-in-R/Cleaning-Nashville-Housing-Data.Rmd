---
title: "Cleaning Nashville Housing Data in R"
author: "Cory Gargan"
date: "2022-12-08"
output:
...
When looking at the Data most of the `SoldAsVacant` Column contained 'yes' or 'no', but there were a few 'Y' and 'N' mixed in.  
...
There were a number of rows with missing `PropertyAddress` data.  I was able to do an InnerJoin with the same data and populate the missed addresses from other rows that contained the same `ParcelID`.  After the missed data was populated I removed any duplicates.  
...
The initial data contained both the `PropertyAddress` & `OwnerAddress` in a single column.  By separating the data by Street, City, and State it becomes more uniform in appearance and usable for analysis. Once the columns were separated I changed any Blank or NULL Owner information to 'N/A' 
...
The last step I took was to ensure there were no errant spaces within the data columns.  I trimmed all leading/trailing whitespace and then exported the `housing` dataframe to a .CSV file for use with Excel or SQL.    
